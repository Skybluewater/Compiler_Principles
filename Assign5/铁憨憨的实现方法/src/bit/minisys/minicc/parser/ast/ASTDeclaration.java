	...
	public static class Builder {
		...
		public void addSpecfiers(Object node) {
			...
			if (node instanceof ASTToken)
				...
				throw new RuntimeException("Declaration's specfiers accepts String only.");
		}
		...
		public void addInitList(Object node) {
			...
			if (node instanceof ASTInitList)
				initLists.add((ASTInitList) node);
			else if (node instanceof List) {
				for (Object element : (List<?>) node) {
					addInitList(element);
				}
				...
				throw new RuntimeException("Declaration's Initlist accepts InitList only.");
		}

		public ASTDeclaration build() {
			return new ASTDeclaration(specfiers, initLists);
		}
	}
